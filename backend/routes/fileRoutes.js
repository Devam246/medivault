...
router.post("/upload", authenticateToken, runUpload, async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      message: error.message || "Upload pipeline failed (blockchain or storage)",
    });
  }
});
...
router.get("/records", async (req, res) => {
  try {
    ...
    console.error("[UPLOAD] List records error:", error);
    res.status(500).json({
      success: false,
